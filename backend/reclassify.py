...
async def process_group(group_key, entries, client, col_knowledge, col_facts, col_style):
    ...
    try:
        ...
        result = await asyncio.to_thread(
            _write_group_sync, conv_id, pair_idx, entries, classification,
            route_facts, route_style, col_knowledge, col_facts, col_style,
        )
        return {**result, "error": False}
    except Exception as e:
        import traceback
        log.error(f"GROUP CRASH [{conv_id}/{pair_idx}]: {type(e).__name__}: {e}\n{traceback.format_exc()}")
        return {"facts": False, "style": False, "chunks": len(entries), "error": True}
...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        ...
        stats = {"exchanges": 0, "chunks": 0, "facts": 0, "style": 0, "errors": 0}

        for batch_start in range(0, total, BATCH_SIZE):
            batch_end = min(batch_start + BATCH_SIZE, total)
            batch     = sorted_groups[batch_start:batch_end]
            tasks     = [process_group(k, e, client, col_knowledge, col_facts, col_style)
                         for k, e in batch]
            ...
            for r in results:
                ...
                if r["facts"]:  stats["facts"]  += 1
                if r["style"]:  stats["style"]  += 1
                if r["error"]:  stats["errors"] += 1

            pct = batch_end / total * 100
            print(f"  {batch_end}/{total} ({pct:.0f}%) | "
                  f"exchanges={stats['exchanges']} facts={stats['facts']} "
                  f"style={stats['style']} errors={stats['errors']}")
    ...
    print(f"  Facts created        : {stats['facts']}")
    print(f"  Style created        : {stats['style']}")
