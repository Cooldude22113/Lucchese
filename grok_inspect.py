...
for filename in glob.glob("prod-grok-*.json"):
    ...
    for entry in conversations:
        ...
        for r in responses:
            ...
            parent = resp.get("parent_response_id")
            if parent is None:
                null_parent += 1
            ...
            # Optional fields
            if resp.get("thinking_start_time"):
                has_thinking += 1
            if resp.get("web_search_results"):
                has_web_search += 1
            if resp.get("file_attachments"):
                has_file_attachments += 1
            if resp.get("card_attachments_json"):
                has_card_attachments += 1
            if resp.get("agent_thinking_traces"):
                has_agent_traces += 1
            if resp.get("steps"):
                has_steps += 1
